...
1. type, there are four types `db3` ,`db3_db` , `db3_permission` and `db3_external`, the `db3` is a user type , the `db3_db` is a database type and the `db3_permission` is a permission type
