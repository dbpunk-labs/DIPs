...
1. type, there are two types `db3` and `db3_db` , the `db3` is a user type and the `db3_db` is a database type
...
| delegation_payment_chain_id |STRING| the delegation payment chain id|Optional|
| delegation_nonce |int64| the nonce of delegation|Optional|
...
| member_required | boolean| only the member of database can write data to the database|No|
...
flowchart TD
    root(Root Hash) --> version
    root(Root Hash) --> chain_id
    root(Root Hash) --> delegation_payment_chain_id
    root(Root Hash) --> delegation_nonce
    root(Root Hash) --> type
    root(Root Hash) --> data
```
use the native signature scheme of delegation_payment_chain_id to sign the hashed message
